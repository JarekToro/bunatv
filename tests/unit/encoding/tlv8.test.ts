...
describe('TLV8 Core Functionality', () => {
  describe('TLV8.encode', () => {
    ...
    it('should fragment values larger than 255 bytes', () => {
      ...
      const expectedFirst = Buffer.concat([Buffer.from([0x02, 0xff]), Buffer.alloc(255, 0x31)])
...
describe('TlvBuilder', () => {
  ...
  it('should handle all convenience methods', () => {
    ...
    expect(data[TlvValue.Error]).toEqual(Buffer.from([ErrorCode.Authentication]))
    ...
    expect(data[TlvValue.Identifier]).toEqual(Buffer.from('test-device', 'utf8'))
  })
...
describe('Edge Cases and Error Handling', () => {
  ...
  it('should validate type boundaries correctly', () => {
    ...
    expect(() => TLV8.encode(-1, validBuffer)).toThrow('TLV type must be 0-255, got -1')
    expect(() => TLV8.encode(256, validBuffer)).toThrow('TLV type must be 0-255, got 256')
  })
