...
import { Method, State, TLV8, TlvBuilder, TlvValue } from '@/core/encoding/tlv8.ts'
...
describe('Enhanced OPACK Implementation', () => {
  describe('OPACK Message Creation and Analysis', () => {
    it('should handle complex TLV8 data with multiple fields', () => {
      ...
      expect(decodedTlv8[TlvValue.Method]).toEqual(Buffer.from([Method.PairSetup]))
      expect(decodedTlv8[TlvValue.SeqNo]).toEqual(Buffer.from([State.M3]))
      expect(decodedTlv8[TlvValue.Identifier]).toEqual(Buffer.from('test-device-12345', 'utf8'))
  ...
  describe('OPACK Encoding/Decoding Edge Cases', () => {
    ...
    it('should detect and handle malformed OPACK data', () => {
      const malformedData = Buffer.from('{"invalid": "json", "missing": "_pd"}')
    ...
    describe('Buffer Serialization Edge Cases', () => {
      ...
      it('should handle arrays with mixed types including Buffers', () => {
        ...
        expect(decoded.mixedArray[3].nestedBuffer).toEqual(Buffer.from([12, 13, 14]))
      })
    ...
    describe('Object Reference Support', () => {
      describe('Decoding with Object References (0xA0-0xC4)', () => {
        it('should decode small object reference (0xA0)', () => {
          ...
          expect(decoded._c._idsID).toBe('4874b16b-4b77-4998-b9d5-091ef8cf486a')
  ...
  describe('BigInt Support', () => {
    ...
    it('should validate BigInt fits in 8 bytes', () => {
      ...
      expect(() => OPACK.encode({ val: tooLarge })).toThrow('BigInt out of range')
    })

    it('should reject negative BigInt', () => {
      const negative = -1n

      expect(() => OPACK.encode({ val: negative })).toThrow('BigInt out of range')
      expect(() => OPACK.encode({ val: negative })).toThrow('cannot be negative')
    })
    ...
    it('should handle session ID round-trip without precision loss', () => {
      ...
      const combinedSessionId = (BigInt(remoteSessionId) << 32n) | BigInt(localSessionId)
