...
const DOUBLE_KEY_IN = { 1: Buffer.from([0x31, 0x31, 0x31]), 4: Buffer.from([0x32, 0x32, 0x32]) } // OrderedDict([(1, b"111"), (4, b"222")])
const DOUBLE_KEY_OUT = Buffer.from([0x01, 0x03, 0x31, 0x31, 0x31, 0x04, 0x03, 0x32, 0x32, 0x32]) // b"\x01\x03\x31\x31\x31\x04\x03\x32\x32\x32"
...
describe('PyATV TLV8 Compatibility Tests', () => {
  ...
  describe('stringify function', () => {
    ...
    it('test_stringify_seqno', () => {
      expect(stringify({ [TlvValue.SeqNo]: Buffer.from([0x01]) })).toBe('SeqNo=M1')
      expect(stringify({ [TlvValue.SeqNo]: Buffer.from([0x02]) })).toBe('SeqNo=M2')
      expect(stringify({ [TlvValue.SeqNo]: Buffer.from([0x03]) })).toBe('SeqNo=M3')
      expect(stringify({ [TlvValue.SeqNo]: Buffer.from([0x04]) })).toBe('SeqNo=M4')
      expect(stringify({ [TlvValue.SeqNo]: Buffer.from([0x05]) })).toBe('SeqNo=M5')
      expect(stringify({ [TlvValue.SeqNo]: Buffer.from([0x06]) })).toBe('SeqNo=M6')
    })
    ...
    it('test_stringify_backoff', () => {
      const result = stringify({ [TlvValue.BackOff]: Buffer.from([0x02, 0x00]) })
    ...
    it('test_stringify_remaining_short', () => {
      ...
      for (const value of values) {
        const result = stringify({ [value]: Buffer.from([0x00, 0x01, 0x02, 0x03]) })
    ...
    it('test_stringify_multiple', () => {
      ...
      expect(result).toBe('Method=PairSetup, SeqNo=M1, Error=BackOff, BackOff=1s')
    })
