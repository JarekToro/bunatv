...
describe("HKDF Key Derivation Operations", () => {
  ...
  describe("Key Derivation", () => {
    it("should derive keys with HKDF-SHA512", async () => {
      ...
      const { result: derivedKey, duration } =
        await PerformanceUtils.measureAsync("hkdf_derive", () =>
  ...
  describe("HAP-Specific Key Derivation", () => {
    it("should derive pair-setup encryption key", async () => {
      ...
      const { result: encryptionKey, duration } =
        await PerformanceUtils.measureAsync("hap_pair_setup_key", () =>
    ...
    it("should derive session keys for pair-verify", async () => {
      ...
      const { result: sessionKeys, duration } =
        await PerformanceUtils.measureAsync("hap_session_keys", () =>
      ...
      expect(
        ComparisonUtils.arraysEqual(sessionKeys.readKey, sessionKeys.writeKey)
      ).toBe(false);

      CryptoLoggingUtils.logPerformance(
        "HAP session keys derivation",
        duration
      );
    });
    ...
    it("should derive keys with correct HAP salts", async () => {
      ...
      expect(ComparisonUtils.arraysEqual(pairSetupKey, controllerKey)).toBe(
        false
      );
      expect(ComparisonUtils.arraysEqual(pairSetupKey, accessoryKey)).toBe(
        false
      );
      expect(ComparisonUtils.arraysEqual(controllerKey, accessoryKey)).toBe(
        false
      );
    });
  ...
  describe("Error Handling", () => {
    it("should handle invalid input key material", async () => {
      ...
      await expect(
        HkdfUtils.derive(null as any, emptySalt, info, 32)
      ).rejects.toThrow();
    });
    ...
    it("should handle null/undefined inputs gracefully", async () => {
      ...
      await ErrorTestUtils.expectAsyncError(() =>
        HkdfUtils.derive(null as any, salt, info, 32)
      );

      await ErrorTestUtils.expectAsyncError(() =>
        HkdfUtils.derive(ikm, null as any, info, 32)
      );

      await ErrorTestUtils.expectAsyncError(() =>
        HkdfUtils.derive(ikm, salt, null as any, 32)
      );
    });
  });
    ...
    it("should handle HAP protocol message flows", async () => {
      ...
      for (let i = 0; i < allKeys.length; i++) {
        ...
        for (let j = i + 1; j < allKeys.length; j++) {
          expect(ComparisonUtils.arraysEqual(allKeys[i]!, allKeys[j]!)).toBe(
            false
          );
        }
      }
