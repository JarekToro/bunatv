...
describe("X25519 Cryptographic Operations", () => {
  ...
  describe("Key Generation", () => {
    it("should generate valid X25519 key pairs", async () => {
      ...
      // Validate key sizes
      KeyValidationUtils.validateX25519Keys(
        keyPair.privateKey,
        keyPair.publicKey
      );
      ...
      expect(
        TestVectorUtils.arraysEqual(keyPair.privateKey, keyPair.publicKey)
      ).toBe(false);

      // Performance check (should complete quickly)
      ComparisonUtils.expectWithinTimeLimit(
        duration,
        100,
    ...
    it("should generate unique key pairs", async () => {
      ...
      expect(
        TestVectorUtils.arraysEqual(keyPair1.privateKey, keyPair2.privateKey)
      ).toBe(false);
      expect(
        TestVectorUtils.arraysEqual(keyPair1.publicKey, keyPair2.publicKey)
      ).toBe(false);
    });
  ...
  describe("Deterministic Test Vector Validation", () => {
    it("should handle known test vectors correctly", async () => {
      const { clientKeyPair, serverKeyPair, expectedSharedSecret } =
        X25519_TEST_VECTORS;
      ...
      expect(
        TestVectorUtils.arraysEqual(clientSharedSecret, serverSharedSecret)
      ).toBe(true);

      // Should match expected deterministic value
      expect(
        TestVectorUtils.arraysEqual(clientSharedSecret, expectedSharedSecret)
      ).toBe(true);
    ...
    it("should verify deterministic key generation", () => {
      ...
      expect(
        TestVectorUtils.arraysEqual(
          regeneratedClient.privateKey,
          clientKeyPair.privateKey
        )
      ).toBe(true);
      expect(
        TestVectorUtils.arraysEqual(
          regeneratedClient.publicKey,
          clientKeyPair.publicKey
        )
      ).toBe(true);
      expect(
        TestVectorUtils.arraysEqual(
          regeneratedServer.privateKey,
          serverKeyPair.privateKey
        )
      ).toBe(true);
      expect(
        TestVectorUtils.arraysEqual(
          regeneratedServer.publicKey,
          serverKeyPair.publicKey
        )
      ).toBe(true);
    });
    ...
    it("should compute shared secrets consistently", async () => {
      ...
      const secrets = await Promise.all([
        X25519Utils.computeSharedSecret(
          clientKeyPair.privateKey,
          serverKeyPair.publicKey
        ),
        X25519Utils.computeSharedSecret(
  ...
  describe("Key Exchange Operations", () => {
    it("should perform ECDH key exchange correctly", async () => {
      ...
      expect(
        TestVectorUtils.arraysEqual(aliceSharedSecret, bobSharedSecret)
      ).toBe(true);
      ...
      ComparisonUtils.expectWithinTimeLimit(
        aliceDuration,
        50,
        "X25519 key exchange (Alice)"
      );
      ComparisonUtils.expectWithinTimeLimit(
        bobDuration,
        50,
        "X25519 key exchange (Bob)"
      );

      CryptoLoggingUtils.logPerformance(
        "X25519 key exchange",
        (aliceDuration + bobDuration) / 2
      );
    });
    ...
    it("should handle sync key exchange", async () => {
      ...
      const { result: aliceSharedSecret, duration: aliceDuration } =
        PerformanceUtils.measureSync("x25519_key_exchange_sync_alice", () =>
          X25519Utils.computeSharedSecretSync(
            aliceKeyPair.privateKey,
            bobKeyPair.publicKey
          )
        );
      ...
      const { result: bobSharedSecret, duration: bobDuration } =
        PerformanceUtils.measureSync("x25519_key_exchange_sync_bob", () =>
          X25519Utils.computeSharedSecretSync(
            bobKeyPair.privateKey,
            aliceKeyPair.publicKey
          )
      ...
      ComparisonUtils.expectWithinTimeLimit(
        aliceDuration,
        25,
        "X25519 sync key exchange (Alice)"
      );
      ComparisonUtils.expectWithinTimeLimit(
        bobDuration,
        25,
        "X25519 sync key exchange (Bob)"
      );
    });
    ...
    it("should handle invalid public keys", async () => {
      ...
      try {
        const result = await X25519Utils.computeSharedSecret(
    ...
    it("should handle concurrent key exchanges", async () => {
      ...
      for (let i = 0; i < sharedSecrets.length; i++) {
        ...
        for (let j = i + 1; j < sharedSecrets.length; j++) {
          expect(
            TestVectorUtils.arraysEqual(sharedSecrets[i]!, sharedSecrets[j]!)
          ).toBe(false);
        }
      }
    });
  ...
  describe("Error Handling", () => {
    ...
    it("should throw CryptoError for invalid operations", async () => {
      ...
      await expect(
        X25519Utils.computeSharedSecret(invalidKey, validKey)
    ...
    it("should handle null/undefined inputs gracefully", async () => {
      ...
      await expect(
        X25519Utils.computeSharedSecret(null as any, validKey)
      ).rejects.toThrow();
      await expect(
        X25519Utils.computeSharedSecret(validKey, null as any)
      ).rejects.toThrow();
    });
    ...
    it("should validate seed size for deterministic generation", () => {
      ...
      for (const invalidSeed of invalidSeeds) {
        expect(() => X25519Utils.generateKeyPairFromSeed(invalidSeed)).toThrow(
          CryptoError
        );
      }
    });
  ...
  describe("Performance Benchmarks", () => {
    ...
    it("should compare sync vs async performance", async () => {
      ...
      expect(syncExchange.avgDuration).toBeLessThan(
        asyncExchange.avgDuration * 2
      ); // Allow some variance
    });
  });
    ...
    it("should not leak memory during repeated operations", async () => {
      ...
      const { memoryDelta } = await MemoryUtils.monitorOperation(async () => {
        for (let i = 0; i < iterations; i++) {
          ...
          expect(sharedSecret.length).toBe(
            HAP_TEST_CONSTANTS.X25519_SHARED_SECRET_SIZE
          );
        }
      });
  ...
  describe("Bun Runtime Integration", () => {
    ...
    it("should use optimal implementation for Bun", async () => {
      if (BunTestUtils.isBun()) {
        logger.debug(
          `Running X25519 tests in Bun: ${BunTestUtils.getBunVersion()}`
        );
  ...
  describe("HAP Protocol Integration", () => {
    it("should work with HAP ephemeral key exchange", async () => {
      ...
      expect(clientSharedSecret.length).toBe(
        HAP_TEST_CONSTANTS.X25519_SHARED_SECRET_SIZE
      );
    });
    ...
    it("should be compatible with HAP key format requirements", () => {
      ...
      expect(clientKeyPair.privateKey.length).toBe(
        HAP_TEST_CONSTANTS.X25519_PRIVATE_KEY_SIZE
      );
      expect(clientKeyPair.publicKey.length).toBe(
        HAP_TEST_CONSTANTS.X25519_PUBLIC_KEY_SIZE
      );
      expect(serverKeyPair.privateKey.length).toBe(
        HAP_TEST_CONSTANTS.X25519_PRIVATE_KEY_SIZE
      );
      expect(serverKeyPair.publicKey.length).toBe(
        HAP_TEST_CONSTANTS.X25519_PUBLIC_KEY_SIZE
      );
    });
  });
