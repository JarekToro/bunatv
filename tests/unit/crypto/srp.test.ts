...
describe("SRP Authentication Operations", () => {
  ...
  describe("Key Generation", () => {
    it("should generate SRP keys", async () => {
      ...
      const { result: key, duration } = await PerformanceUtils.measureAsync(
        "srp_keygen",
  ...
  describe("SRP Client Operations", () => {
    it("should create SRP client", () => {
      ...
      const { result: client, duration } = PerformanceUtils.measureSync(
        "srp_client_create",
  ...
  describe("SRP Server Operations", () => {
    it("should create SRP server", () => {
      ...
      const { result: server, duration } = PerformanceUtils.measureSync(
        "srp_server_create",
    ...
    it("should create server with same credentials as client", () => {
      ...
      const server = SrpUtils.createServer(
        username,
        password,
        salt,
        serverSecretKey
  ...
  describe("SRP Verifier Computation", () => {
    it("should compute SRP verifier", () => {
      ...
      const { result: verifier, duration } = PerformanceUtils.measureSync(
        "srp_verifier",
  ...
  describe("HAP Protocol Integration", () => {
    ...
    it("should handle HAP salt requirements", () => {
      ...
      const hapSalt = Buffer.from(
        RandomTestDataUtils.randomBytes(HAP_TEST_CONSTANTS.SRP_SALT_SIZE)
      );
  ...
  describe("Cross-Compatibility with pyatv", () => {
    it("should produce deterministic outputs for known inputs", () => {
      ...
      const client1 = SrpUtils.createClient(
        username,
        password,
        salt,
        secretKey
      );
      const client2 = SrpUtils.createClient(
        username,
        password,
        salt,
        secretKey
