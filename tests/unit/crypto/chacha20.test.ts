...
describe("ChaCha20-Poly1305 Encryption Operations", () => {
  ...
  describe("Encryption and Decryption", () => {
    it("should encrypt data with ChaCha20-Poly1305", async () => {
      ...
      const { result: ciphertext, duration } =
        await PerformanceUtils.measureAsync("chacha20_encrypt", () =>
    ...
    it("should decrypt data with ChaCha20-Poly1305", async () => {
      ...
      // Then decrypt
      const { result: decrypted, duration } =
        await PerformanceUtils.measureAsync("chacha20_decrypt", () =>
    ...
    it("should encrypt/decrypt with additional authenticated data (AAD)", async () => {
      ...
      // Encrypt with AAD
      const ciphertext = await ChaCha20Utils.encrypt(
        key,
        nonce,
        plaintext,
        aad
    ...
    it("should encrypt/decrypt synchronously", () => {
      ...
      const { result: ciphertext, duration: encryptDuration } =
        PerformanceUtils.measureSync("chacha20_encrypt_sync", () =>
          ChaCha20Utils.encryptSync(key, nonce, plaintext)
        );

      const { result: decrypted, duration: decryptDuration } =
        PerformanceUtils.measureSync("chacha20_decrypt_sync", () =>
      ...
      CryptoLoggingUtils.logPerformance(
        "ChaCha20-Poly1305 sync encrypt",
        encryptDuration
      );
      CryptoLoggingUtils.logPerformance(
        "ChaCha20-Poly1305 sync decrypt",
        decryptDuration
      );
    });
  ...
  describe("Authentication and Integrity", () => {
    it("should reject ciphertext with modified auth tag", async () => {
      ...
      await expect(
        ChaCha20Utils.decrypt(key, nonce, corruptedCiphertext)
      ).rejects.toThrow();
    });
    ...
    it("should reject with wrong AAD", async () => {
      ...
      await expect(
        ChaCha20Utils.decrypt(key, nonce, ciphertext, wrongAad)
      ).rejects.toThrow();
    });
  ...
  describe("Error Handling", () => {
    ...
    it("should handle null/undefined inputs gracefully", async () => {
      ...
      await ErrorTestUtils.expectAsyncError(() =>
        ChaCha20Utils.encrypt(key, nonce, null as any)
      );
    });
  });
  ...
  describe("Performance Benchmarks", () => {
    ...
    it("should benchmark different message sizes", async () => {
      ...
      for (const size of sizes) {
        ...
        const totalDuration = avgDuration * iterations;
        const mbps =
          (size * iterations) / (totalDuration / 1000) / (1024 * 1024);
        logger.debug(
          `ChaCha20 ${size}B: ${avgDuration.toFixed(2)}ms avg, ${mbps.toFixed(2)} MB/s`
        );
  ...
  describe("Memory Usage", () => {
    it("should not leak memory during repeated operations", async () => {
      ...
      CryptoLoggingUtils.logMemoryUsage(
        "ChaCha20 repeated operations",
        memoryUsed,
        iterations
      );
    ...
    it("should handle large message encryption without excessive memory", async () => {
      ...
      CryptoLoggingUtils.logMemoryUsage(
        "ChaCha20 large message encryption",
        memoryUsed,
        1
      );
  ...
  describe("Bun Runtime Integration", () => {
    it("should work correctly in Bun runtime", async () => {
      ...
      logger.debug(
        `Running ChaCha20-Poly1305 tests in Bun: ${runtimeInfo.bunVersion}`
      );
