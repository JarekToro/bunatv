...
import { CryptoLoggingUtils, PerformanceUtils } from '../../helpers/crypto-test-utils'
...
describe('SRP M1-M6 Message Validation Tests', () => {
  ...
  describe('M1 Message Validation - Initial Pairing Request', () => {
    it('should validate valid M1 pair-setup message structure', () => {
      const { result: messageBuffer, duration } = PerformanceUtils.measureSync('M1_encode', () =>
        HapMessageUtils.buildMessage(M1_MESSAGE_VECTORS.validMessage)
      )

      const { result: decoded, duration: decodeDuration } = PerformanceUtils.measureSync(
        'M1_decode',
      ...
      expect(
        HapMessageUtils.validateMessage(decoded, M1_MESSAGE_VECTORS.validMessage.expectedStructure)
      ).toBe(true)
    ...
    it('should validate M1 pair-setup with auth message', () => {
      const messageBuffer = HapMessageUtils.buildMessage(M1_MESSAGE_VECTORS.validWithAuth)
      const decoded = TLV8.decodeObject(messageBuffer) as AssertedTLV8

      expect(decoded[TlvValue.Method].readUInt8(0)).toBe(Method.PairSetupWithAuth)
      expect(decoded[TlvValue.SeqNo].readUInt8(0)).toBe(State.M1)
    })

    it('should validate M1 pair-verify message with public key', () => {
      const messageBuffer = HapMessageUtils.buildMessage(M1_MESSAGE_VECTORS.pairVerifyM1)
      const decoded = TLV8.decodeObject(messageBuffer) as AssertedTLV8

      expect(decoded[TlvValue.Method].readUInt8(0)).toBe(Method.PairVerify)
      expect(decoded[TlvValue.SeqNo].readUInt8(0)).toBe(State.M1)
      expect(decoded[TlvValue.PublicKey]).toBeDefined()
      expect(decoded[TlvValue.PublicKey].length).toBe(HAP_TEST_CONSTANTS.X25519_PUBLIC_KEY_SIZE)
    })

    it('should build M1 message using TlvBuilder', () => {
      const message = new TlvBuilder().method(Method.PairSetup).seqNo(State.M1).build()
    ...
    it('should stringify M1 message correctly', () => {
      const messageBuffer = HapMessageUtils.buildMessage(M1_MESSAGE_VECTORS.validMessage)
      const decoded = TLV8.decodeObject(messageBuffer) as AssertedTLV8
    ...
    describe('M1 Error Cases', () => {
      it('should handle M1 message missing Method field', () => {
        const messageBuffer = HapMessageUtils.buildMessage(M1_MESSAGE_VECTORS.invalid.missingMethod)
        const decoded = TLV8.decodeObject(messageBuffer) as AssertedTLV8
      ...
      it('should handle M1 message missing SeqNo field', () => {
        const messageBuffer = HapMessageUtils.buildMessage(M1_MESSAGE_VECTORS.invalid.missingSeqNo)
        const decoded = TLV8.decodeObject(messageBuffer) as AssertedTLV8
      ...
      it('should handle M1 message with wrong sequence number', () => {
        const messageBuffer = HapMessageUtils.buildMessage(M1_MESSAGE_VECTORS.invalid.wrongSeqNo)
        const decoded = TLV8.decodeObject(messageBuffer) as AssertedTLV8
      ...
      it('should handle M1 message with invalid method value', () => {
        const messageBuffer = HapMessageUtils.buildMessage(M1_MESSAGE_VECTORS.invalid.invalidMethod)
        const decoded = TLV8.decodeObject(messageBuffer) as AssertedTLV8
  ...
  describe('M2 Message Validation - Server Challenge Response', () => {
    it('should validate valid M2 pair-setup message structure', () => {
      const messageBuffer = HapMessageUtils.buildMessage(M2_MESSAGE_VECTORS.validPairSetup)
      const decoded = TLV8.decodeObject(messageBuffer) as AssertedTLV8
      ...
      expect(decoded[TlvValue.Salt].length).toBe(HAP_TEST_CONSTANTS.SRP_SALT_SIZE)
    ...
    it('should validate M2 pair-verify message structure', () => {
      const messageBuffer = HapMessageUtils.buildMessage(M2_MESSAGE_VECTORS.validPairVerify)
      const decoded = TLV8.decodeObject(messageBuffer) as AssertedTLV8
    ...
    it('should validate M2 error response structure', () => {
      const messageBuffer = HapMessageUtils.buildMessage(M2_MESSAGE_VECTORS.errorResponse)
      const decoded = TLV8.decodeObject(messageBuffer) as AssertedTLV8

      expect(decoded[TlvValue.Method].readUInt8(0)).toBe(Method.PairSetup)
      expect(decoded[TlvValue.SeqNo].readUInt8(0)).toBe(State.M2)
      expect(decoded[TlvValue.Error]).toBeDefined()
      expect(decoded[TlvValue.Error].readUInt8(0)).toBe(ErrorCode.Authentication)
    })
    ...
    describe('M2 Error Cases', () => {
      ...
      it('should handle M2 message with invalid salt size', () => {
        ...
        expect(decoded[TlvValue.Salt].length).not.toBe(HAP_TEST_CONSTANTS.SRP_SALT_SIZE)
      })
    })
  })
  ...
  describe('M3 Message Validation - Client Proof', () => {
    it('should validate valid M3 pair-setup message structure', () => {
      const messageBuffer = HapMessageUtils.buildMessage(M3_MESSAGE_VECTORS.validPairSetup)
      const decoded = TLV8.decodeObject(messageBuffer) as AssertedTLV8
    ...
    it('should validate M3 pair-verify message structure', () => {
      const messageBuffer = HapMessageUtils.buildMessage(M3_MESSAGE_VECTORS.validPairVerify)
      const decoded = TLV8.decodeObject(messageBuffer) as AssertedTLV8
    ...
    describe('M3 Error Cases', () => {
      it('should handle M3 message missing proof', () => {
        const messageBuffer = HapMessageUtils.buildMessage(M3_MESSAGE_VECTORS.invalid.missingProof)
        const decoded = TLV8.decodeObject(messageBuffer) as AssertedTLV8
  ...
  describe('M4 Message Validation - Server Proof', () => {
    it('should validate valid M4 pair-setup message structure', () => {
      const messageBuffer = HapMessageUtils.buildMessage(M4_MESSAGE_VECTORS.validPairSetup)
      const decoded = TLV8.decodeObject(messageBuffer) as AssertedTLV8
    ...
    it('should validate M4 pair-verify message structure', () => {
      const messageBuffer = HapMessageUtils.buildMessage(M4_MESSAGE_VECTORS.validPairVerify)
      const decoded = TLV8.decodeObject(messageBuffer) as AssertedTLV8
    ...
    it('should validate M4 error response structure', () => {
      const messageBuffer = HapMessageUtils.buildMessage(M4_MESSAGE_VECTORS.errorResponse)
      const decoded = TLV8.decodeObject(messageBuffer) as AssertedTLV8
    ...
    describe('M4 Error Cases', () => {
      it('should handle M4 message missing proof', () => {
        const messageBuffer = HapMessageUtils.buildMessage(M4_MESSAGE_VECTORS.invalid.missingProof)
        const decoded = TLV8.decodeObject(messageBuffer) as AssertedTLV8
  ...
  describe('M5 Message Validation - Client Encrypted Data', () => {
    it('should validate valid M5 message structure', () => {
      const messageBuffer = HapMessageUtils.buildMessage(M5_MESSAGE_VECTORS.valid)
      const decoded = TLV8.decodeObject(messageBuffer) as AssertedTLV8
      ...
      // Validate using helper function
      expect(
        HapMessageUtils.validateMessage(decoded, M5_MESSAGE_VECTORS.valid.expectedStructure)
      ).toBe(true)
    })
  ...
  describe('M6 Message Validation - Server Encrypted Response', () => {
    it('should validate valid M6 message structure', () => {
      const messageBuffer = HapMessageUtils.buildMessage(M6_MESSAGE_VECTORS.valid)
      const decoded = TLV8.decodeObject(messageBuffer) as AssertedTLV8
      ...
      // Validate using helper function
      expect(
        HapMessageUtils.validateMessage(decoded, M6_MESSAGE_VECTORS.valid.expectedStructure)
      ).toBe(true)
    })

    it('should validate M6 error response structure', () => {
      const messageBuffer = HapMessageUtils.buildMessage(M6_MESSAGE_VECTORS.errorResponse)
      const decoded = TLV8.decodeObject(messageBuffer) as AssertedTLV8
  ...
  describe('Complete HAP Flow Validation', () => {
    it('should validate successful pair-setup flow', () => {
      ...
      for (let i = 0; i < flow.length; i++) {
        ...
        expect(HapMessageUtils.validateMessage(decoded, messageVector!.expectedStructure)).toBe(
          true
        )
      }
    })
  ...
  describe('TLV8 Error Handling', () => {
    ...
    it('should handle duplicate TLV entries correctly', () => {
      ...
      const result1 = TLV8.decodeObject(duplicateVectors.sameType) as AssertedTLV8
      ...
      const result2 = TLV8.decodeObject(duplicateVectors.fragmented) as AssertedTLV8
      ...
      const { avgDuration } = PerformanceUtils.benchmarkSync(
        ...
        () => {
          ...
          for (const messageVector of flow) {
            ...
            // Validate message
            HapMessageUtils.validateMessage(decoded, messageVector.expectedStructure)
          }
        },
        iterations
      )
