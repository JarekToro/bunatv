...
describe("Ed25519 Cryptographic Operations", () => {
  ...
  describe("Key Generation", () => {
    it("should generate valid Ed25519 key pairs", async () => {
      ...
      // Validate key sizes
      KeyValidationUtils.validateEd25519Keys(
        keyPair.privateKey,
        keyPair.publicKey
      );
  ...
  describe("Deterministic Test Vector Validation", () => {
    it("should handle known test vectors correctly", async () => {
      ...
      const signature = await Ed25519Utils.sign(
        testMessage,
      ...
      const isValid = await Ed25519Utils.verify(
        signature,
        testMessage,
      ...
      CryptoLoggingUtils.logValidation(
        "Ed25519 test vector validation",
        isValid
      );
    });
    ...
    it("should work with client and server key pairs from fixtures", async () => {
      ...
      const clientSignature = await Ed25519Utils.sign(
        testMessage,
      ...
      const serverSignature = await Ed25519Utils.sign(
        testMessage,
  ...
  describe("Signing Operations", () => {
    it("should sign messages correctly", async () => {
      ...
      const { result: signature, duration } =
        await PerformanceUtils.measureAsync("ed25519_sign", () =>
    ...
    it("should sign large messages efficiently", async () => {
      ...
      const { result: signature, duration } =
        await PerformanceUtils.measureAsync("ed25519_sign_large", () =>
    ...
    it("should handle batch verification efficiently", async () => {
      ...
      const results = await Promise.all(
        signatures.map((sig, i) =>
          Ed25519Utils.verify(sig, messages[i]!, keyPair.publicKey)
        )
      );
  ...
  describe("Error Handling", () => {
    ...
    it("should throw CryptoError for invalid operations", async () => {
      ...
      await expect(
        Ed25519Utils.sign(message, invalidKey)
    ...
    it("should handle null/undefined inputs gracefully", async () => {
      ...
      await expect(
        Ed25519Utils.sign(null as any, keyPair.privateKey)
      ).rejects.toThrow();
      await expect(Ed25519Utils.sign(message, null as any)).rejects.toThrow();
      await expect(
        Ed25519Utils.verify(null as any, message, keyPair.publicKey)
      ).rejects.toThrow();
    });
  });
  ...
  describe("Bun Runtime Integration", () => {
    ...
    it("should use optimal implementation for Bun", async () => {
      if (BunTestUtils.isBun()) {
        logger.debug(
          `Running Ed25519 tests in Bun: ${BunTestUtils.getBunVersion()}`
        );
  ...
  describe("HAP Protocol Integration", () => {
    ...
    it("should handle HAP identifier format", async () => {
      const keyPair = await Ed25519Utils.generateKeyPair();
      const hapIdentifier = Buffer.from(
        HAP_TEST_CONSTANTS.CLIENT_IDENTIFIER,
        "utf8"
      );
