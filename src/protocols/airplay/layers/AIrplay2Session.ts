  ...
  implements Protocol<Airplay2SessionEvents>
{
  ...
  private async setupRtspSession(): Promise<void> {
    ...
    logger.debug(
      ...
      "Remote control setup complete"
    );

    logger.debug("Event channel connected");

    const eventEncryptionLayer = new ChaCha20EncryptionLayer({
      format: NonceFormat.Hap,
    });
    const eventDataKeys = HkdfUtils.deriveAirPlayEventKeysSync(
      this.sharedSecret
    );
    ...
    const encryptionLayer = new ChaCha20EncryptionLayer({
      format: NonceFormat.Hap,
    });
    const dataKeys = HkdfUtils.deriveAirPlayDataStreamKeysSync(
      this.sharedSecret,
      streamConfig.seed
    );
