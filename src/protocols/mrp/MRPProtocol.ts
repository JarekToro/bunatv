...
export class MRPProtocol extends EventEmitter<MRPProtocolEvents> {
  ...
  private async tearDown() {
    logger.debug({ count: this.outstanding.size }, 'Cleaning up outstanding MRP requests')
    ...
    logger.debug(
      {
        ...
        messageType: ProtocolMessageExtensionDisplayNameMap[payload.extensionType],
      },
   ...
   */
  private async receive(identifier: string, timeout: number): Promise<ProtocolMessageResult> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.outstanding.set(identifier, { resolve, reject, timeout: timeoutHandle })
    })
  }
  ...
  messageReceived(buffer: Buffer): void {
    try {
      ...
      logger.trace(
        { type: resolved.extensionType, identifier: resolved.message.identifier },
        'MRP message received'
      )

      // If the message identifier is outstanding, then someone is
      // waiting for the response so we save it here
      const identifier = resolved.message.identifier || `type_${resolved.message.type}`
      ...
      this.emit(
        ...
        'message:' + ProtocolMessageExtensionDisplayNameMap[resolved.extensionType!],
        resolved
      )
    } catch (error) {
