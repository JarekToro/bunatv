  ...
  implements Protocol<CompanionProtocolEvents>
{
  ...
  private readonly hapFramedChannel = new HapFramedChannel(this.transport, this.encryption)
  ...
  static async create(device: DiscoveredDevice, storage: Storage): Promise<CompanionProtocol> {
  ...
  private constructor(
    ...
    this.credentialStore = this.storage.getCredentialStore(device.identifier, 'companion')
  ...
  private async _connect(
    ...
    await this.transport.connect(this.connectionInfo.address, this.connectionInfo.port, options)
    ...
    if (!this.authService.isAuthenticated) {
      const authSession = await this.authService.authenticate(credentials, authOptions)
    ...
    if (!authSessionKey) {
      throw new Error('No authentication session key available after authentication')
    }
  ...
  async connect(
    ...
    try {
      await this.recoveryManager.runWithRecovery(() => this._connect(authOptions, options), {
        maxAttempts: 3,
        delay: 300,
      })
    } catch (error) {
  ...
  async sendCommandRaw<T extends CompanionCommand>(command: T): Promise<CommandResponseOf<T>> {
    return this.session.sendCommandRaw(command)
  }
  async sendCommand<T extends CompanionCommand>(command: T): Promise<CommandOutputOf<T>> {
