...
export class HAPAuthenticationService extends EventEmitter<HapAuthEvents> {
  ...
  constructor(
    ...
    logger.info(
      ...
      "HAPAuthenticationService initialized"
    );
  }
  ...
  private setState(newState: HAPAuthState): void {
    ...
    logger.debug(
      { oldState: oldInternalState, newState },
      "HAP authentication state changed"
    );
  }
    ...
    logger.info(
      {
        ...
        hasPinCallback: !!callbacks?.onPinRequired,
      },
  ...
  async pair(
    ...
    const credentials = await this.startPairingInternal(
      callbacks.onPinRequired,
      clientId
    );
    logger.info(
      { clientId: credentials.clientId },
      "HAP pairing completed successfully"
    );
    return credentials;
  }
   ...
   * Verify existing credentials
   */
  private async verifyCredentialsInternal(
    credentials: HAPCredentials
  ): Promise<SessionKeys> {
    if (this.internalState !== HAPAuthState.Idle) {
      logger.error(
    ...
    logger.debug(
      { clientId: credentials.clientId },
      "Initializing credential verification flow"
    );
    ...
    this.verificationHandler = new VerificationFlowHandler(
      credentials,
  ...
  private async handleFrame(frame: HAPFrame): Promise<void> {
    ...
    try {
      ...
      if (tlv[TlvValue.Error]) {
        ...
        logger.error(
          { errorCode, state: this.internalState },
          "Received HAP error in frame"
        );
      ...
      logger.error(
        { error, frameType: frame.type },
        "Error handling HAP frame"
      );
   ...
   */
  private async sendAuthFlowMessage(
    frameType: HAPFrameType,
    payload: Buffer
  ): Promise<void> {
  ...
  private reset(): void {
    logger.debug(
      { previousState: this.internalState },
      "Resetting HAP authentication service"
    );
...
class PairingFlowHandler {
  ...
  async createM3(pin: string): Promise<Buffer> {
    ...
    this.srpClient = SrpUtils.createClient(
      "Pair-Setup",
      pin,
      this.serverSalt,
      secretKey
  ...
  private createCompanionAdditionalData(): Buffer {
    ...
    if (!this.config.clientDeviceInfo) {
      throw new Error(
        "Client device info required for Companion additional data"
      );
    }
  ...
  private async createM5(srpKey: Buffer): Promise<Buffer> {
    ...
    const encrypted = await ChaCha20Utils.encrypt(
      encryptionKey,
      nonce.subarray(0, 12),
      innerTlv
    );
...
class VerificationFlowHandler {
  ...
  async processM2AndCreateM3(tlvData: TlvData): Promise<Buffer> {
    ...
    this.sessionKeys = HkdfUtils.deriveCompanionSessionKeysSync(
      this.sharedSecret
    );
    logger.info(
      {
