...
export class ChaCha20EncryptionLayer {
  ...
  encrypt(data: Buffer, aad?: Buffer): Buffer {
    if (!this.isEnabled || !this.sessionKeys) {
      logger.trace(
        ...
        "Encryption bypassed - layer disabled"
      );
    ...
    try {
      ...
      const nonce = BunOptimizedUtils.createNonce(
        this.sendNonce++,
        this.nonceFormat
      );
  ...
  decrypt(data: Buffer, aad?: Buffer): Buffer {
    if (!this.isEnabled || !this.sessionKeys) {
      logger.trace(
        { dataLength: data.length },
        "Decryption bypassed - layer disabled"
      );
      return data; // Pass through if not enabled
    }
    ...
    try {
      ...
      logger.error(
        { error, nonce: this.receiveNonce - 1 },
        "Decryption failed"
      );
      this._state = EncryptionState.Error;
  ...
  resetNonces(): void {
    logger.debug(
      {
        previousSendNonce: this.sendNonce,
        previousReceiveNonce: this.receiveNonce,
      },
      "Resetting nonces"
    );
    this.sendNonce = 0;
