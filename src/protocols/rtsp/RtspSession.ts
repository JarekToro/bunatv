import type {
  HttpFramedChannel,
  HttpResponse,
} from "../airplay/layers/HttpFramedChannel.ts";
...
export class RtspSession {
  ...
  private async request(
    ...
    if (response.statusCode !== 200) {
      throw new Error(
        `RTSP ${method} failed: ${response.statusCode} ${response.statusText}`
      );
    }
  ...
  async setupRemoteControl(
    setupInfo: RemoteControlSetupInfo
  ): Promise<RtspSetupResponse> {
  ...
  async setupDataStream(config: DataStreamConfig): Promise<RtspSetupResponse> {
    ...
    const parsed = Plist.decode(response.body) as Record<string, unknown>;
    const streams = parsed.streams as
