  ...
  return pinoPretty({
    ...
    messageFormat: (log, messageKey, _, extras) => {
      ...
      const bgColorPairs = [
        ...
        { bg: COLORS.bgMagentaBright, text: COLORS.white, name: 'bgMagentaBright' },
        { bg: COLORS.bgYellowBright, text: COLORS.black, name: 'bgYellowBright' },
        { bg: COLORS.bgRedBright, text: COLORS.white, name: 'bgRedBright' },
      ]

      const module = (log as any).module ? ` ${(log as any).module} ` : undefined
      let moduleColorFunc = (text: string) => text.toString()
      if (module && !colorModuleMap.has(module)) {
        const pair = bgColorPairs[colorModuleMap.size % bgColorPairs.length] ?? {
          bg: COLORS.bgWhite,
          text: COLORS.black,
          name: 'bgWhite',
        }
        colorModuleMap.set(module, (text: string | number) => pair.bg(pair.text(text)))
