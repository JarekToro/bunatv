...
function hasUnknownFields(obj: unknown, path = '', seen = new WeakSet()): string[] {
  const paths: string[] = []
  ...
  if ('_unknownFields' in obj) {
    ...
    if (unknown && typeof unknown === 'object' && Object.keys(unknown).length > 0) {
      for (const fieldNum of Object.keys(unknown)) {
        paths.push(path ? `${path}._unknownFields.${fieldNum}` : `_unknownFields.${fieldNum}`)
      }
    }
  }
...
export class MRPCapture {
  ...
  constructor() {
    ...
    if (this.mode !== 'NONE') {
      logger.info({ mode: this.mode, sessionId: this.sessionId }, 'MRP capture enabled')
    }
  }
  ...
  private async ensureInitialized(): Promise<void> {
    ...
    if (!this.initPromise) {
      this.initPromise = (async () => {
        ...
        logger.debug({ sessionDir: this.sessionDir }, 'Capture session created')
      })()
    }
  ...
  private async writeEntry(
    ...
    const typeName =
      type !== -1
        ? ((ProtocolMessageExtensionDisplayNameMap as Record<number, string>)[type] ?? 'Unknown')
        : 'Unknown'
  ...
  async captureSent(raw: Buffer | Uint8Array, type: number, decoded: unknown): Promise<void> {
    await this.writeEntry(raw, 'out', type, decoded)
  }

  async captureReceived(raw: Buffer, type: number | undefined, decoded: unknown): Promise<void> {
