...
const outputFormatType = new EnumType<OutputFormat>([
  "text",
  "json",
  "table",
] as const);
...
const cli = new Command()
  ...
  .globalOption("-v, --verbose", "Verbose output with detailed information", {
    default: false,
  })
  ...
  .globalAction((options) => {
    setGlobalLogLevel(
      options.debug === true ? "debug" : (options.debug ?? "silent")
    );
  });
