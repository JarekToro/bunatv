...
export const discoverCommand = new Command<GlobalOptions>()
  ...
  .option('-t, --timeout <seconds:number>', 'Discovery timeout in seconds', { default: 5 })
  .option('-f, --filter <text:string>', 'Filter devices by name or identifier')
  .option('--companion-only', 'Show only Companion-capable devices', { default: false })
          ...
          const formattedDevices = devicesWithStatus.map(device => {
            ...
            if (options.verbose) {
              ...
              if (device.services?.companionLink) {
                info['Companion Port'] = device.services.companionLink.port.toString()
              }
            }
