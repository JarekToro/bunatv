...
export const wizardCommand = new Command<GlobalOptions>()
  ...
  .action(async (options) => {
    ...
    output.info(
      "This wizard will help you discover and pair with your Apple TV.\n"
    );
    ...
    await withErrorHandling(output, async () => {
      ...
      if (credentialsSaved) {
        ...
        output.warn(
          "⚠️  Warning: Credentials may not have been saved properly"
        );
      }
    });
    ...
    output.listItem(
      `bunatv pair ${selectedDevice} - Pair with a specific device`
    );
    output.listItem(`bunatv control ${selectedDevice} volume get - Get volume`);
    output.listItem(
      `bunatv control ${selectedDevice} remote select - Press select`
    );
    output.listItem(
      `bunatv control ${selectedDevice} status - Get device status`
    );
    output.info("\nEnjoy using BunATV!");
  });
