...
export const remoteCommand = new Command<GlobalOptions & ControlOptions>()
  ...
  .action(async (options, buttonArg: string) => {
    ...
    const device = (options as unknown as Record<string, unknown>).device as string | undefined
    const hold = (options as unknown as Record<string, unknown>).hold as boolean | undefined
    const double = (options as unknown as Record<string, unknown>).double as boolean | undefined
    ...
    const timeout =
      ((options as unknown as Record<string, unknown>).timeout as number | undefined) ?? 10
    const autoRecover =
      ((options as unknown as Record<string, unknown>).autoRecover as boolean | undefined) ?? true
