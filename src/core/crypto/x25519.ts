...
export class X25519Utils {
  ...
  static async generateKeyPair(): Promise<{ privateKey: Uint8Array; publicKey: Uint8Array }> {
    ...
    logger.debug('Using @noble/curves/x25519 (more reliable than WebCrypto for X25519)...')
    try {
      ...
      try {
        ...
        logger.debug('WebCrypto key generation succeeded but raw export may fail')
  ...
  static computeSharedSecretSync(privateKey: Uint8Array, publicKey: Uint8Array): Uint8Array {
    ...
    try {
      ...
      logger.debug(`computeSharedSecretSync: Shared secret length: ${secret.length}`)
