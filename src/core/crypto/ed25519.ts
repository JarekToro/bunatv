...
export class Ed25519Utils {
  ...
  static async generateKeyPair(): Promise<{ privateKey: Uint8Array; publicKey: Uint8Array }> {
    ...
    try {
      ...
      logger.debug({ error: webCryptoError }, 'WebCrypto failed, falling back to @noble/curves...')
  ...
  static async verify(
    ...
    logger.debug(`Verify: sig=${signature.length}B, data=${data.length}B, key=${publicKey.length}B`)
   ...
   */
  static generateKeyPairSync(): { privateKey: Uint8Array; publicKey: Uint8Array } {
    ...
    try {
      ...
      logger.debug(`generateKeyPairSync: Private key length: ${privateKey.length}`)
      logger.debug(`generateKeyPairSync: Public key length: ${publicKey.length}`)
      ...
      logger.debug(`generateKeyPairSync: Final private key length: ${result.privateKey.length}`)
      logger.debug(`generateKeyPairSync: Final public key length: ${result.publicKey.length}`)
