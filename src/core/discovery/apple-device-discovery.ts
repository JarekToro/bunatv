...
export class AppleTVDiscoveryService {
  ...
  constructor() {
    ...
    this.network = new MDNSNetworkDiscovery(response => this.registry.updateFromResponse(response))
  }
   ...
   */
  async getAppleDeviceByHostname(hostname: string): Promise<AppleDevice | null> {
    await this.registry.ensureLoaded()
    // Check if device exists and is not expired
  ...
  getAirPlayServices(): AirPlayService[] {
    const instances = this.registry.getServiceInstances(APPLE_SERVICE_TYPES.AIRPLAY)
  ...
  getRAOPServices(): RAOPService[] {
    const instances = this.registry.getServiceInstances(APPLE_SERVICE_TYPES.RAOP)
    return instances.map(instance => {
  ...
  getCompanionLinkServices(): CompanionLinkService[] {
    const instances = this.registry.getServiceInstances(APPLE_SERVICE_TYPES.COMPANION_LINK)
    return instances.map(instance => {
      if (!isCompanionLinkMetadata(instance.txt)) {
        console.warn(`Invalid Companion Link metadata for ${instance.instanceName}`)
      }
  ...
  getDeviceInfoServices(): DeviceInfoService[] {
    const instances = this.registry.getServiceInstances(APPLE_SERVICE_TYPES.DEVICE_INFO)
    return instances.map(instance => {
      if (!isDeviceInfoMetadata(instance.txt)) {
        console.warn(`Invalid Device Info metadata for ${instance.instanceName}`)
      }
   ...
   */
  private _getAppleDevices(deviceType: 'appletv' | 'homepod' | 'mac'): AppleDevice[] {
    ...
    for (const airplay of airplayServices) {
      ...
      const raop = this._findRelatedService<RAOPService>(name, APPLE_SERVICE_TYPES.RAOP)
   ...
   */
  private _findRelatedService<T extends { instanceName: string; serviceType: string }>(
    deviceName: string,
    serviceType: string
  ): T | undefined {
