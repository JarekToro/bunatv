...
> = T extends string | symbol
  ? (...args: any[]) => void
  : (...args: EventEmitter.ArgumentMap<Exclude<T, string | symbol>>[Extract<K, keyof T>]) => boolean

export class EmitterEx<EventTypes extends EventEmitter.ValidEventTypes = string | symbol>
    ...
    if (options?.filter) {
      const listener = ((...args: any[]) => {
        if ((options.filter as any)(...args)) {
          this.removeListener(event, listener as EventEmitter.EventListener<EventTypes, U>)
          ;(fn as any)(...args)
        }
    ...
    return new Promise((resolve, reject) => {
      ...
      const listener = ((...args: any[]) => {
        if (!options?.filter || (options.filter as any)(...args)) {
          cleanup()
          resolve(args as Parameters<EventEmitter.EventListener<EventTypes, U>>)
        }
      }) as EventEmitter.EventListener<EventTypes, U>
