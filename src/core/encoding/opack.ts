...
export class OPACK {
  ...
  static encode(obj: Record<string, any>): Buffer {
    try {
      logger.debug(`OPACK.encode: Encoding object with ${Object.keys(obj).length} keys`)
   ...
   */
  private static checkAndReuseObject(packedBytes: Buffer, objectList: Buffer[]): Buffer {
    ...
    const existingIndex = objectList.findIndex(obj => obj.equals(packedBytes))
  ...
  private static packBigInt(value: bigint): Buffer {
    ...
    if (value > MAX_64BIT) {
      throw new Error(`BigInt out of range: ${value} (must be 0 to 0xFFFFFFFFFFFFFFFF)`)
    }
   ...
   */
  private static packDict(data: Record<string, any>, objectList: Buffer[]): Buffer {
  ...
  private static unpackValue(reader: BufferReader, objectList: any[]): any {
    ...
    // Float32 (0x35) - 4 bytes
      ...
      value = new DataView(buffer.buffer, buffer.byteOffset, buffer.byteLength).getFloat32(0, true) // little-endian
    }

    // Float64 (0x36) - 8 bytes
    else if (byte === 0x36) {
      const buffer = reader.readBuffer(8)
      value = new DataView(buffer.buffer, buffer.byteOffset, buffer.byteLength).getFloat64(0, true) // little-endian
    }
    ...
    // Length bytes: byte & 0xF (1, 2, 3, or 4 bytes for length field)
      ...
      if (sizeBytes === 1) len = reader.readUInt8()
        ...
        len = reader.readUInt8() | (reader.readUInt8() << 8) | (reader.readUInt8() << 16)
    ...
    // Dictionaries: 0xE0-0xEF
      ...
      throw new Error(`Unpack not implemented for byte: 0x${byte.toString(16).padStart(2, '0')}`)
    }
...
export class OPACKMESSAGE {
   ...
   */
  static createPairingMessage(tlv8Data: Buffer, transactionId?: number): Buffer {
    logger.debug(
