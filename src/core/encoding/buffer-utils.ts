...
export class BufferWriter {
  ...
  constructor(
    ...
    this.buffer = pool
      ? pool.acquire(initialSize)
      : Buffer.allocUnsafe(initialSize);
  }
  ...
  private ensureCapacity(bytes: number): void {
    ...
    if (required > this.buffer.length) {
      ...
      const newBuffer = this.pool
        ? this.pool.acquire(newSize)
        : Buffer.allocUnsafe(newSize);
